...
class CityService(
    ...
    suspend fun getAllCities(pagination: PaginationDto): PageResponseDto<CityResponseDto> = cityRepository.findAll(pagination)
    ...
    ): PageResponseDto<CityResponseDto> = cityRepository.findByCountryId(countryId, pagination)

    suspend fun searchCitiesByName(
        name: String,
        pagination: PaginationDto
    ): PageResponseDto<CityResponseDto> = cityRepository.findByName(name, pagination)
...
private fun CityCreateDto.toResponseDto(id: UUID) =
    CityResponseDto(
        id = id,
        countryId = countryId,
        nameRu = nameRu,
