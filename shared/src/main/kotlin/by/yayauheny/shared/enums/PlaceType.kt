...
@Serializable
enum class PlaceType(val id: String, val ruName: String, val enName: String) {
    ...
    companion object {
        fun fromString(value: String?): PlaceType =
            entries.find { it.id == value } ?: OTHER
    }

    fun getLocalizedName(locale: String): String =
        if (locale.equals("ru", ignoreCase = true)) ruName else enName
}
